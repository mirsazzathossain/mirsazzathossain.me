...
<img
  src="/images/content/group-theory-foundations/group_theory.png"
  alt="Diving into Group Theory: Foundations for Equivariant Neural Networks"
  sizes="100vw"
  width="100%"
  height="auto"
...
Let's say we have the set $\{0, 1, 2, \ldots, n-1\}$ and we want to find the order of the element 1. We know that the identity element of the group $(\mathbb{Z}/n\mathbb{Z}, +)$ is 0. So, we need to find the smallest positive integer $n$ such that $1^n = 0$. Let's try to find the order of the element 1 for $n = 5$.
